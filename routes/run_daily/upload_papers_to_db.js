...
function addData(response_text) {
    ...
    while (total_paper != paper_amount) {
        ...
        //ADD TYPE if there it one
        paper_type_section = getStr(response_text.indexOf(paper_title), response_text, "class=\"primary-subject", 23, "</div>");
        type_count = (paper_author_section.match(/;/g) || []).length;
        //Get the first author
        type = getStr(0, paper_type_section, "\">", 2, "(");
        //Creates string with all authors in author section, adds comma if there's more than one
        if (type_count > 0) {
            type += ", ";
            type_list = paper_type_section.split(";");
            type_list = type_list.slice(1,);
            for (i in type_list) {
                if (i != type_list.length -1){
                    type += getStr(0, type_list[i], " ", 1, "(") + ", ";
                } else {
                    type += getStr(0, type_list[i], " ", 1, "(");        
                }
            }
